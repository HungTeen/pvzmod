...
public class PVZFeatures {
	...
	public static final ConfiguredFeature<?, ?> PATCH_CHOMPER_PLANT = register("patch_chomper_plant", Feature.RANDOM_PATCH.configured((new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(BlockRegister.CHOMPER.get().defaultBlockState()), SimpleBlockPlacer.INSTANCE)).tries(64).whitelist(ImmutableSet.of(Blocks.GRASS_BLOCK)).noProjection().build()).decorated(Features.Placements.HEIGHTMAP_DOUBLE_SQUARE).chance(32));
	public static final ConfiguredFeature<?, ?> TREES_NUT = register("nut_tree", NUT_TREE.decorated(Placement.COUNT_EXTRA.configured(new AtSurfaceWithExtraConfig(1, 0.05F, 1))));
