...
public class BalloonItem extends Item {
    ...
    public void appendHoverText(ItemStack stack, Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
        ...
        tooltip.add(new TranslatableComponent("tooltip.pvz.balloon").withStyle(ChatFormatting.AQUA));
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        if(PlayerUtil.getResource(playerIn, Resources.SUN_NUM) >= SUN_COST){
            if(! playerIn.level.isClientSide){
                ...
                playerIn.addEffect(new MobEffectInstance(Effects.SLOW_FALLING, EFFECT_CD, 10));
