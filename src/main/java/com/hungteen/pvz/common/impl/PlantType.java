...
public abstract class PlantType extends PAZType implements IPlantType {
	...
	/* other data */
	protected IEssenceType plantEssence = EssenceType.APPEASE;
	...
	private void occurDifficulty(int dif) {
		this.occurDifficulty = dif;
	}
	...
	public static final class PlantFeatures{
		...
		public PlantFeatures maxLevel(int lvl) {
			this.maxLevel = lvl;
			return this;
		}

		public PlantFeatures difficulty(int dif) {
			this.occurDifficulty = dif * ConfigUtil.getIncDifficulty();
			return this;
		}

		public PlantFeatures invasionWeight(int weight) {
			this.randomInvasionWeight = weight;
			return this;
		}

		public PlantFeatures waveWeight(int weight) {
						...
						if(element.isJsonObject()){
							element.getAsJsonObject().entrySet().forEach(entry1 -> {
								...
								if(opt.get() instanceof PlantType && element1.isJsonObject()){
									...
									//occur difficulty.
									final int occurDif = JSONUtils.getAsInt(obj, StringUtil.JSON_OCCUR_DIFFICULTY, 0);
									if(occurDif > 0){
										type.occurDifficulty(occurDif);
									}
								} else{
