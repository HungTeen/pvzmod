	...
	public PlayerInventoryContainer(int id, PlayerEntity player) {
		...
		this.player.getCapability(CapabilityHandler.PLAYER_DATA_CAPABILITY).ifPresent((l)->{
			...
			for(int i=0;i<6;i++) {
				for(int j=0;j<9;j++) {
					int cnt = i*9+j;
					if(inv.getSlotNum()<cnt) this.addSlot(new LockedSlot(inv.getInventory(), i*9+j , 24 + j * 18 , 29 + i * 18));
					else this.addSlot(new CardSlot(inv.getInventory(), i*9+j , 24 + j * 18 , 29 + i * 18));
				}
			}
    	...
    	this.player.getCapability(CapabilityHandler.PLAYER_DATA_CAPABILITY).ifPresent((l)->{
			...
			for(int i=0;i<6;i++) {
				for(int j=0;j<9;j++) {
					...
					if(inv.getSlotNum()<pos) this.inventorySlots.set(i*9+j,new LockedSlot(inv.getInventory(), pos , 24 + j * 18 , 29 + i * 18));
					else this.inventorySlots.set(i*9+j,new CardSlot(inv.getInventory(), pos , 24 + j * 18 , 29 + i * 18));
				}
			}
