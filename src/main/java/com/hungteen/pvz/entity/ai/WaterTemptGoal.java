...
public class WaterTemptGoal extends Goal {
	...
	public WaterTemptGoal(CreatureEntity creatureIn, double speedIn, boolean scaredByPlayerMovementIn,
		...
		if (!(creatureIn.getNavigator() instanceof SwimmerPathNavigator)) {
	...
	public boolean shouldExecute() {
		if (this.delayTemptCounter > 0) {
			...
			if (this.closestPlayer == null) {
				...
				return this.isTempting(this.closestPlayer.getHeldItemMainhand())
						|| this.isTempting(this.closestPlayer.getHeldItemOffhand());
			}
		}
	}
	...
	public boolean shouldContinueExecuting() {
		if (this.isScaredByPlayerMovement()) {
			if (this.creature.getDistanceSq(this.closestPlayer) < 36.0D) {
				if (this.closestPlayer.getDistanceSq(this.targetX, this.targetY,
						this.targetZ) > 0.010000000000000002D) {
					return false;
				}

				if (Math.abs((double) this.closestPlayer.rotationPitch - this.pitch) > 5.0D
						|| Math.abs((double) this.closestPlayer.rotationYaw - this.yaw) > 5.0D) {
					return false;
				}
			} else {
	 ...
	 */
	public void tick() {
		this.creature.getLookController().setLookPositionWithEntity(this.closestPlayer,
