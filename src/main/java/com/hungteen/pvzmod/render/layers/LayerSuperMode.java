...
public class LayerSuperMode implements LayerRenderer<EntityLiving> {
	private static final ResourceLocation SUPER = new ResourceLocation(Reference.MODID + ":" + "textures/entity/layer/super_armor.png");
	private final RenderLiving livingRenderer;
	private final ModelBase livingModel;

	public LayerSuperMode(RenderLiving RendererIn) {
	...
	public void doRenderLayer(EntityLiving entitylivingbaseIn, float limbSwing, float limbSwingAmount,
		...
		if(entitylivingbaseIn instanceof EntityPlantBase) {//植物放大招
			if(!((EntityPlantBase) entitylivingbaseIn).isPlantInSuperMode())
				return ;
		}
		else if(entitylivingbaseIn instanceof EntityZombieBase) {//僵尸
			if(((EntityZombieBase) entitylivingbaseIn).getZombieType()!=EntityZombieBase.Type.SUPER)
				return ;
			if(((EntityZombieBase) entitylivingbaseIn).getIsInivs()) {//隐形不渲染
				return ;
			}
		}
