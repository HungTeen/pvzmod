...
public class LayerSunLight implements LayerRenderer<EntityLiving> {
	private static final ResourceLocation SUPER = new ResourceLocation(Reference.MODID + ":" + "textures/entity/layer/sun_light.png");
	private final RenderLiving livingRenderer;
	private final ModelBase livingModel;

	public LayerSunLight(RenderLiving RendererIn) {
	...
	public void doRenderLayer(EntityLiving entitylivingbaseIn, float limbSwing, float limbSwingAmount,
		...
		if(entitylivingbaseIn instanceof EntityGenPlantBase) {
			if(!((EntityGenPlantBase) entitylivingbaseIn).getIsGenTime()) return ;
		}
