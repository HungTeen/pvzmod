...
public class LayerCold implements LayerRenderer<EntityLiving> {
	private static final ResourceLocation SUPER = new ResourceLocation(Reference.MODID + ":" + "textures/entity/layer/cold_armor.png");
	private final RenderLiving livingRenderer;
	private final ModelBase livingModel;

	public LayerCold(RenderLiving RendererIn) {
	...
	public void doRenderLayer(EntityLiving entitylivingbaseIn, float limbSwing, float limbSwingAmount,
		...
		if(!((EntityZombieBase) entitylivingbaseIn).getIsCold()&&!((EntityZombieBase) entitylivingbaseIn).getIsFrozen()) {
			return ;
		}
