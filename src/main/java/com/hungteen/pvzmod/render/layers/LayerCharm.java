...
public class LayerCharm implements LayerRenderer<EntityLiving> {
	private static final ResourceLocation SUPER = new ResourceLocation(Reference.MODID + ":" + "textures/entity/layer/charm.png");
	private final RenderLiving livingRenderer;
	private final ModelBase livingModel;

	public LayerCharm(RenderLiving RendererIn) {
	...
	public void doRenderLayer(EntityLiving entitylivingbaseIn, float limbSwing, float limbSwingAmount,
		...
		if(entitylivingbaseIn instanceof EntityZombieBase) {
			if(!((EntityZombieBase) entitylivingbaseIn).getIsCharmed()) return ;
		}
		else if(entitylivingbaseIn instanceof EntityPlantBase) {
			if(entitylivingbaseIn instanceof EntityHypnoShroom) {
				
			}else {
				if(!((EntityPlantBase) entitylivingbaseIn).getIsCharmed()) return ;
			}
		}
		else{
			System.out.println("ERROR layer owner?");
			return ;
		}
		...
		GlStateManager.translate(f * 0.003F, f * 0.003F, 0.0F);
