...
public class PVZExplosion {
	...
	public PVZExplosion(World world, Entity exploder, Entity projectile, double x, double y, double z,
			double primaryDamage, double secondaryDamage, double primaryRadius, double secondaryRadius,
			double blockDamageFactor) {
		...
		this.explosionDummy = new Explosion(world, exploder, x, y, z, (float) Math.max(primaryRadius, secondaryRadius),
				false, this.damagesTerrain);
	}
		...
		for (int j = -radius; j < radius; ++j) {
			for (int k = -radius; k < radius; ++k) {
				for (int l = -radius; l < radius; ++l) {
					if (j == -radius || j == radius - 1 || k == -radius || k == radius - 1 || l == -radius
							|| l == radius - 1) {
						...
						for (int i = 0; i < steps && explosionPower > 0.0; i++) {
							...
							double distance = this.position.distanceTo(
									new Vec3d(blockpos.getX() + 0.5, blockpos.getY() + 0.5, blockpos.getZ() + 0.5));
							if (distance <= primaryRadius)
								explosionPower = primaryDamage;
							else if (distance <= secondaryRadius)
								explosionPower = secondaryDamage
										+ ((distance - primaryRadius) / (secondaryRadius - primaryRadius))
							...
							if (iblockstate.getMaterial() != Material.AIR) {
								resistance = iblockstate.getBlock().getExplosionResistance(world, blockpos, exploder,
										explosionDummy);
...
//                               f -= (f2 + 0.3F) * 0.3F;
								...
								if (explosionPower - explosionDamping > 0.0f
										&& resistance < (explosionPower - explosionDamping) * blockDamageFactor
										&& (this.exploder == null
												|| this.exploder.canExplosionDestroyBlock(explosionDummy, this.world,
														blockpos, iblockstate, (float) explosionPower))) {
									set.put(blockpos, distance);
									if (prevPos == null
											|| !(prevPos.getX() == blockpos.getX() && prevPos.getY() == blockpos.getY()
													&& prevPos.getZ() == blockpos.getZ())) {
		...
		// <debug>
		// System.out.println(String.format("min = (%d, %d, %d); max = (%d, %d,
		// %d)",min[0], min[1], min[2], max[0], max[1], max[2]));
		// </debug>
		...
		List<Entity> list = this.world.getEntitiesWithinAABBExcludingEntity(this.projectile,
		...
		PVZDamageSource tgs;
		if(this.dmgSrc==null) {
    		tgs = PVZDamageSource.causeExplosionDamage(projectile,exploder);
    	} else {
    		tgs = PVZDamageSource.copyWithNewEnt(this.dmgSrc, projectile, exploder);
    	}
		...
		for (int k2 = 0; k2 < list.size(); ++k2) {
			...
			if (!entity.isImmuneToExplosions()) {
				...
				// Check distance
				double distance = this.position
						.distanceTo(new Vec3d(entity.posX, entity.posY + entity.getEyeHeight(), entity.posZ));
				if (distance <= primaryRadius)
					damage = primaryDamage;
				else if (distance <= secondaryRadius)
					damage = secondaryDamage + ((distance - primaryRadius) / (secondaryRadius - primaryRadius))
							* (primaryDamage - secondaryDamage);
				else
					damage = 0.0;
				if (damage > 0.0) {
