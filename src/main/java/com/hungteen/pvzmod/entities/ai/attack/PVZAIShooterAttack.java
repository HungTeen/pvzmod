...
public class PVZAIShooterAttack extends EntityAIBase{
	...
	public PVZAIShooterAttack(IShooter shooter) {
...
//		this.attackTime=0;
	...
	public void updateTask() {
		this.attackTime++;
		if(this.attackTime>=this.shooter.getShootSpeed()) {
			this.attackTime=0;
	...
	private boolean checkTarget()
	{
		if(EntityUtil.checkCanEntityAttack(this.attacker,target)) {
			double dy=target.posY-this.attacker.posY;
			if(EntityShooterBase.checkY(dy)) {
