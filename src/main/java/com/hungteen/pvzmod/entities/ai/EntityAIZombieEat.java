...
public class EntityAIZombieEat extends EntityAIBase
{
	World world;
    ...
    public EntityAIZombieEat(EntityCreature creature, double speedIn, boolean useLongMemory)
    {
    ...
    {
		if(this.attacker instanceof EntityZombieBase) {
			if(((EntityZombieBase) this.attacker).getIsButter()||((EntityZombieBase) this.attacker).getIsFrozen()){
				return ;
			}
		}
        EntityLivingBase entitylivingbase = this.attacker.getAttackTarget();
	...
	protected void checkAndPerformAttack(EntityLivingBase target, double p_190102_2_) {
        ...
        {
        	if(this.attacker instanceof EntityZombieBase) {
                this.attackTick = ((EntityZombieBase) this.attacker).getAttackTick();
